...
// Constructor
//-----------------------------------------------------------------------------
CPUTDropdown::CPUTDropdown(const std::string ControlName, CPUTControlID id, CPUTFont *pFont):mVertexStride(0),
    mVertexOffset(0),
    mSelectedItem(-1),
    mHighlightedItem(0),
    mbSizeDirty(false),
    ...
    mpFont(pFont)
{
    ...
    mpSelectedItemCopy = CPUTText::Create(pFont);
...
//-----------------------------------------------------------------------------
CPUTResult CPUTDropdown::AddSelectionItem(const std::string Item, bool bIsSelected)
{
    ...
    CPUTText *pNewItem = CPUTText::Create(mpFont);
    ...
    if(bIsSelected)
    {
        mSelectedItem = (int) mpListOfSelectableItems.size()-1;    // set the highlighted element to this item
    }
...
//-----------------------------------------------------------------------------
void CPUTDropdown::NumberOfSelectableItems(int &count)
{
    count = (int)mpListOfSelectableItems.size();
}

// Return the index of the currently selected item
//-----------------------------------------------------------------------------
void CPUTDropdown::GetSelectedItem(int &index)
{
    index = mSelectedItem;
}
...
//-----------------------------------------------------------------------------
void CPUTDropdown::GetSelectedItem(std::string &Item)
{
    if(-1 != mSelectedItem)
    {
        mpListOfSelectableItems[mSelectedItem]->GetString(Item);
    }
}
...
{
    if( index < mpListOfSelectableItems.size() )
    {
        mSelectedItem = index;
    }
...
//-----------------------------------------------------------------------------
void CPUTDropdown::SetHighlightedItem(const int index)
{
    int listSize =  (int) mpListOfSelectableItems.size(); 
    if (index>-1 && index < listSize )
    {
        mHighlightedItem = index;
    }
...
void CPUTDropdown::DeleteSelectionItem(const unsigned int index)
{
    ...
    if(mSelectedItem > (int)itemIndex)
    {
        mSelectedItem--;
        mHighlightedItem--;
    }
    if(mSelectedItem >= (int)mpListOfSelectableItems.size())
    {
        mSelectedItem = (UINT) mpListOfSelectableItems.size()-1;
...
//-----------------------------------------------------------------------------
void CPUTDropdown::DeleteSelectionItem(const std::string string)
{
    std::string itemText;
...
//-----------------------------------------------------------------------------
CPUTEventHandledCode CPUTDropdown::HandleMouseEvent(int x, int y, int wheel, CPUTMouseState state, CPUTEventID eventID)
{
    ...
    if(trayOpen == false)
    {
        if(eventID == CPUT_EVENT_DOWN && state & CPUT_MOUSE_LEFT_DOWN && mouseInTop)
        {
            mControlGuiState = CPUT_DROPDOWN_GUI_MOUSE_PRESSED;
            SetHighlightedItem(mSelectedItem);
            SetFocus(true);
    ...
    else
    {
        ...
        if(eventID == CPUT_EVENT_DOWN)
        {
            ...
            {
                mHighlightedItem = GetItem(x, y);
                return CPUT_EVENT_HANDLED;
            }
            else //check if focus here
            {
                mControlGuiState = CPUT_DROPDOWN_GUI_MOUSE_NEUTRAL;
                mHighlightedItem = mSelectedItem;
                SetFocus(false);
        ...
        if(state & CPUT_MOUSE_LEFT_DOWN || TrayContainsPoint(x, y))
        {
            SetHighlightedItem(GetItem(x, y));
            return CPUT_EVENT_HANDLED;
        }
        if(state & CPUT_MOUSE_LEFT_DOWN && TopContainsPoint(x, y))
        {
            return CPUT_EVENT_HANDLED;
        }
        return CPUT_EVENT_HANDLED;
...
int CPUTDropdown::GetItem(int x, int y)
{
    CPUT_RECT inner, outer;
    CalculateTrayRect(inner, outer);
    int itemWidth, itemHeight;
    CalculateMaxItemSize(itemWidth, itemHeight);
    return (int)( (y - inner.y)  / (float)(itemHeight+2*CPUT_DROPDOWN_TEXT_PADDING));
}
...
void CPUTDropdown::CalculateReadoutTextPosition(int &x, int &y)
{
    ...
    {
        mpListOfSelectableItems[mSelectedItem]->GetDimensions(textDimensions.width, textDimensions.height);
    }
...
{
    return TopContainsPoint(x, y) || TrayContainsPoint(x, y);
}

bool CPUTDropdown::TopContainsPoint(int x, int y)
{
    CPUT_RECT inner, outer;
    CalculateReadoutRect(inner, outer);
    return ((x > outer.x) && (x < outer.x + outer.width) && (y > outer.y) && (y < outer.y + outer.height))
        || ((x > mButtonRect.x) && (x < mButtonRect.x+mButtonRect.width) && (y > mButtonRect.y) && (y < mButtonRect.y+mButtonRect.height));
}

bool CPUTDropdown::TrayContainsPoint(int x, int y)
{
...
void CPUTDropdown::SetPosition(int x, int y)
{
    ...
    if(-1 != mSelectedItem)
    {
        mpListOfSelectableItems[mSelectedItem]->SetPosition(a,b);
    }
...
void CPUTDropdown::Recalculate()
{
    ...
    // Idle button quads
    // left
    AddQuadIntoMirrorBuffer(mpMirrorBufferActive, 0 * 6, (float)mControlDimensions.x, (float)mControlDimensions.y, (float)mpDropdownIdleImageSizeList[0].width, (float)mpDropdownIdleImageSizeList[0].height, mpDropdownUVCoords_active[2 * 0], mpDropdownUVCoords_active[2 * 0 + 1], mColor);
    AddQuadIntoMirrorBuffer(mpMirrorBufferActive, 1 * 6, (float)mControlDimensions.x, (float)mControlDimensions.y + mSmallestTopSizeIdle, (float)mpDropdownIdleImageSizeList[1].width, (float)middleHeight, mpDropdownUVCoords_active[2 * 1], mpDropdownUVCoords_active[2 * 1 + 1], mColor);
    mpDropdownIdleSizeList[1].height = middleHeight;
    AddQuadIntoMirrorBuffer(mpMirrorBufferActive, 2 * 6, (float)mControlDimensions.x, (float)mControlDimensions.y + mSmallestTopSizeIdle + middleHeight, (float)mpDropdownIdleImageSizeList[2].width, (float)mpDropdownIdleImageSizeList[2].height, mpDropdownUVCoords_active[2 * 2], mpDropdownUVCoords_active[2 * 2 + 1], mColor);

    // middle
    AddQuadIntoMirrorBuffer(mpMirrorBufferActive, 3 * 6, (float)mControlDimensions.x + mSmallestLeftSizeIdle, (float)mControlDimensions.y, (float)middleWidth, (float)mpDropdownIdleImageSizeList[3].height, mpDropdownUVCoords_active[2 * 3], mpDropdownUVCoords_active[2 * 3 + 1], mColor);
    mpDropdownIdleSizeList[3].width = middleWidth;
    AddQuadIntoMirrorBuffer(mpMirrorBufferActive, 4 * 6, (float)mControlDimensions.x + mSmallestLeftSizeIdle, (float)mControlDimensions.y + mSmallestTopSizeIdle, (float)middleWidth, (float)middleHeight, mpDropdownUVCoords_active[2 * 4], mpDropdownUVCoords_active[2 * 4 + 1], mColor);
    mpDropdownIdleSizeList[4].width = middleWidth;
    mpDropdownIdleSizeList[4].height = middleHeight;
    AddQuadIntoMirrorBuffer(mpMirrorBufferActive, 5 * 6, (float)mControlDimensions.x + mSmallestLeftSizeIdle, (float)mControlDimensions.y + mSmallestTopSizeIdle + middleHeight, (float)middleWidth, (float)mpDropdownIdleImageSizeList[5].height, mpDropdownUVCoords_active[2 * 5], mpDropdownUVCoords_active[2 * 5 + 1], mColor);
    mpDropdownIdleSizeList[5].width = middleWidth;

    // right
    AddQuadIntoMirrorBuffer(mpMirrorBufferActive, 6 * 6, (float)mControlDimensions.x + mSmallestLeftSizeIdle + middleWidth, (float)mControlDimensions.y, (float)mpDropdownIdleImageSizeList[6].width, (float)mpDropdownIdleImageSizeList[6].height, mpDropdownUVCoords_active[2 * 6], mpDropdownUVCoords_active[2 * 6 + 1], mColor);
    AddQuadIntoMirrorBuffer(mpMirrorBufferActive, 7 * 6, (float)mControlDimensions.x + mSmallestLeftSizeIdle + middleWidth, (float)mControlDimensions.y + mSmallestTopSizeIdle, (float)mpDropdownIdleImageSizeList[7].width, (float)middleHeight, mpDropdownUVCoords_active[2 * 7], mpDropdownUVCoords_active[2 * 7 + 1], mColor);
    mpDropdownIdleSizeList[7].height = middleHeight;
    AddQuadIntoMirrorBuffer(mpMirrorBufferActive, 8 * 6, (float)mControlDimensions.x + mSmallestLeftSizeIdle + middleWidth, (float)mControlDimensions.y + mSmallestTopSizeIdle + middleHeight, (float)mpDropdownIdleImageSizeList[8].width, (float)mpDropdownIdleImageSizeList[8].height, mpDropdownUVCoords_active[2 * 8], mpDropdownUVCoords_active[2 * 8 + 1], mColor);

    // 2 buttons (up and pressed) at same location
    CPUT_RECT rect;
    CalculateButtonRect(rect);
    AddQuadIntoMirrorBuffer(mpMirrorBufferActive, 9 * 6, (float)rect.x, (float)rect.y, (float)mpDropdownIdleImageSizeList[9].width, (float)mpDropdownIdleImageSizeList[9].height, mpDropdownUVCoords_active[2 * 9], mpDropdownUVCoords_active[2 * 9 + 1], mColor);
    AddQuadIntoMirrorBuffer(mpMirrorBufferActive, 10 * 6, (float)rect.x, (float)rect.y, (float)mpDropdownIdleImageSizeList[10].width, (float)mpDropdownIdleImageSizeList[10].height, mpDropdownUVCoords_active[2 * 10], mpDropdownUVCoords_active[2 * 10 + 1], mColor);
    ...
    if(CPUT_CONTROL_ACTIVE == mControlState)
    {
        if((CPUT_DROPDOWN_GUI_MOUSE_PRESSED == mControlGuiState) && (0 != mpListOfSelectableItems.size()))
        {
            ...
            // lm
            AddQuadIntoMirrorBuffer(mpMirrorBufferActive, index * 6, (float)tx, (float)ty, (float)mpDropdownIdleImageSizeList[CTrayLM].width, (float)mTrayDimensions.height, mpDropdownUVCoords_active[2 * CTrayLM], mpDropdownUVCoords_active[2 * CTrayLM + 1], mColor);
            index++;

            // lb
            ty+=mTrayDimensions.height; // inner.height
            AddQuadIntoMirrorBuffer(mpMirrorBufferActive, index * 6, (float)tx, (float)ty, (float)mpDropdownIdleImageSizeList[CTrayLB].width, (float)mpDropdownIdleImageSizeList[CTrayLB].height, mpDropdownUVCoords_active[2 * CTrayLB], mpDropdownUVCoords_active[2 * CTrayLB + 1], mColor);
            index++;

            // mm
            tx+=mpDropdownIdleImageSizeList[CLeftMid].width;
            ty = (float)outer.y;
            AddQuadIntoMirrorBuffer(mpMirrorBufferActive, index * 6, (float)tx, (float)ty, (float)mTrayDimensions.width, (float)mTrayDimensions.height, mpDropdownUVCoords_active[2 * CTrayMM], mpDropdownUVCoords_active[2 * CTrayMM + 1], mColor);
            index++;

            // mb
            ty+=mTrayDimensions.height;
            AddQuadIntoMirrorBuffer(mpMirrorBufferActive, index * 6, (float)tx, (float)ty, (float)mTrayDimensions.width, (float)mpDropdownIdleImageSizeList[CTrayMB].height, mpDropdownUVCoords_active[2 * CTrayMB], mpDropdownUVCoords_active[2 * CTrayMB + 1], mColor);
            index++;
            ...
            ty = (float)outer.y;
            AddQuadIntoMirrorBuffer(mpMirrorBufferActive, index * 6, (float)tx, (float)ty, (float)mpDropdownIdleImageSizeList[CTrayRM].width, (float)mTrayDimensions.height, mpDropdownUVCoords_active[2 * CTrayRM], mpDropdownUVCoords_active[2 * CTrayRM + 1], mColor);
            index++;

            // rb
            ty += mTrayDimensions.height;
            AddQuadIntoMirrorBuffer(mpMirrorBufferActive, index * 6, (float)tx, (float)ty, (float)mpDropdownIdleImageSizeList[CTrayRB].width, (float)mpDropdownIdleImageSizeList[CTrayRB].height, mpDropdownUVCoords_active[2 * CTrayRB], mpDropdownUVCoords_active[2 * CTrayRB + 1], mColor);
            index++;
    ...
    // left
    AddQuadIntoMirrorBuffer(mpMirrorBufferDisabled, 0 * 6, (float)mControlDimensions.x, (float)mControlDimensions.y, (float)mpDropdownIdleImageSizeList[0].width, (float)mpDropdownIdleImageSizeList[0].height, mpDropdownUVCoords_disabled[2 * 0], mpDropdownUVCoords_disabled[2 * 0 + 1], mColor);
    AddQuadIntoMirrorBuffer(mpMirrorBufferDisabled, 1 * 6, (float)mControlDimensions.x, (float)mControlDimensions.y + mSmallestTopSizeIdle, (float)mpDropdownIdleImageSizeList[1].width, (float)middleHeight, mpDropdownUVCoords_disabled[2 * 1], mpDropdownUVCoords_disabled[2 * 1 + 1], mColor);
    mpDropdownIdleSizeList[1].height = middleHeight;
    AddQuadIntoMirrorBuffer(mpMirrorBufferDisabled, 2 * 6, (float)mControlDimensions.x, (float)mControlDimensions.y + mSmallestTopSizeIdle + middleHeight, (float)mpDropdownIdleImageSizeList[2].width, (float)mpDropdownIdleImageSizeList[2].height, mpDropdownUVCoords_disabled[2 * 2], mpDropdownUVCoords_disabled[2 * 2 + 1], mColor);

    // middle
    AddQuadIntoMirrorBuffer(mpMirrorBufferDisabled, 3 * 6, (float)mControlDimensions.x + mSmallestLeftSizeIdle, (float)mControlDimensions.y, (float)middleWidth, (float)mpDropdownIdleImageSizeList[3].height, mpDropdownUVCoords_disabled[2 * 3], mpDropdownUVCoords_disabled[2 * 3 + 1], mColor);
    mpDropdownIdleSizeList[3].width = middleWidth;
    AddQuadIntoMirrorBuffer(mpMirrorBufferDisabled, 4 * 6, (float)mControlDimensions.x + mSmallestLeftSizeIdle, (float)mControlDimensions.y + mSmallestTopSizeIdle, (float)middleWidth, (float)middleHeight, mpDropdownUVCoords_disabled[2 * 4], mpDropdownUVCoords_disabled[2 * 4 + 1], mColor);
    mpDropdownIdleSizeList[4].width = middleWidth;
    mpDropdownIdleSizeList[4].height = middleHeight;
    AddQuadIntoMirrorBuffer(mpMirrorBufferDisabled, 5 * 6, (float)mControlDimensions.x + mSmallestLeftSizeIdle, (float)mControlDimensions.y + mSmallestTopSizeIdle + middleHeight, (float)middleWidth, (float)mpDropdownIdleImageSizeList[5].height, mpDropdownUVCoords_disabled[2 * 5], mpDropdownUVCoords_disabled[2 * 5 + 1], mColor);
    mpDropdownIdleSizeList[5].width = middleWidth;

    // right
    AddQuadIntoMirrorBuffer(mpMirrorBufferDisabled, 6 * 6, (float)mControlDimensions.x + mSmallestLeftSizeIdle + middleWidth, (float)mControlDimensions.y, (float)mpDropdownIdleImageSizeList[6].width, (float)mpDropdownIdleImageSizeList[6].height, mpDropdownUVCoords_disabled[2 * 6], mpDropdownUVCoords_disabled[2 * 6 + 1], mColor);
    AddQuadIntoMirrorBuffer(mpMirrorBufferDisabled, 7 * 6, (float)mControlDimensions.x + mSmallestLeftSizeIdle + middleWidth, (float)mControlDimensions.y + mSmallestTopSizeIdle, (float)mpDropdownIdleImageSizeList[7].width, (float)middleHeight, mpDropdownUVCoords_disabled[2 * 7], mpDropdownUVCoords_disabled[2 * 7 + 1], mColor);
    mpDropdownIdleSizeList[7].height = middleHeight;
    AddQuadIntoMirrorBuffer(mpMirrorBufferDisabled, 8 * 6, (float)mControlDimensions.x + mSmallestLeftSizeIdle + middleWidth, (float)mControlDimensions.y + mSmallestTopSizeIdle + middleHeight, (float)mpDropdownIdleImageSizeList[8].width, (float)mpDropdownIdleImageSizeList[8].height, mpDropdownUVCoords_disabled[2 * 8], mpDropdownUVCoords_disabled[2 * 8 + 1], mColor);

    // 2 buttons (up and pressed) at same location
    CalculateButtonRect(rect);
    AddQuadIntoMirrorBuffer(mpMirrorBufferDisabled, 9 * 6, (float)rect.x, (float)rect.y, (float)mpDropdownIdleImageSizeList[9].width, (float)mpDropdownIdleImageSizeList[9].height, mpDropdownUVCoords_disabled[2 * 9], mpDropdownUVCoords_disabled[2 * 9 + 1], mColor);
    AddQuadIntoMirrorBuffer(mpMirrorBufferDisabled, 10 * 6, (float)rect.x, (float)rect.y, (float)mpDropdownIdleImageSizeList[10].width, (float)mpDropdownIdleImageSizeList[10].height, mpDropdownUVCoords_disabled[2 * 10], mpDropdownUVCoords_disabled[2 * 10 + 1], mColor);
    ...
    if(-1!=mSelectedItem)
    {
        ...
        std::string string;
        mpListOfSelectableItems[mSelectedItem]->GetString(string);
        mpSelectedItemCopy->SetText(string);
        mpSelectedItemCopy->SetPosition(x,y);

        mpListOfSelectableItems[mSelectedItem]->SetPosition(x,y);
    }
...
//--------------------------------------------------------------------------------
void CPUTDropdown::Draw(CPUTGUIVertex *pVertexBufferMirror, UINT *pInsertIndex, UINT pMaxBufferSize)
{
    if(!mControlVisible)
    {
        return;
    ...
    UNREFERENCED_PARAMETER(VertexCopyCount);
    ...
    {
        mpSelectedItemCopy->Draw((CPUTGUIVertex*)pVertexBufferMirror, pInsertIndex, pMaxBufferSize);
    }
    ...
    if((CPUT_DROPDOWN_GUI_MOUSE_PRESSED == mControlGuiState) && (0 != mpListOfSelectableItems.size()))
    {
        for(UINT i=0;i<mpListOfSelectableItems.size();i++)
        {
            mpListOfSelectableItems[i]->Draw((CPUTGUIVertex*)pVertexBufferMirror, pInsertIndex, pMaxBufferSize);
        }
    }
}

CPUTDropdown* CPUTDropdown::Create(const std::string controlName, CPUTControlID controlID, CPUTFont * mpFont)
{
	return new CPUTDropdown(controlName, controlID, mpFont);
}
